...
<a id="auto_attachments_sh_button" title="<?php _e('Auto Attachments Shortcodes','autoa');?>" class="button-secondary" href="#" style="cursor:pointer;">
        <span class="aa-paperclip" alt="<?php _e('Auto Attachments Shortcodes','autoa');?>" ></span> <?php _e('AA Shortcode','autoa');?>
    </a>
<?php  }
...
function button_js() {
	//wp_enqueue_style('modali', plugins_url().'/auto-attachments/includes/js/css/modal.css', __FILE__);
	//Call style proper way
...
function getimages_aa($atts) {
	...
	$gmeta = get_post_meta($post->ID,'aa_post_meta', TRUE);
...
function getmusic_aa($atts) {
	...
	if ($opts['hideaudioall'] == 'no'){
		...
		$musicaa = "";
		if($ashow == 'yes' || $ashow == '') {
			wp_enqueue_script('mel',''.$urlp.'/js/mel/mediaelement-and-player.min.js',array('jquery'));
			wp_enqueue_script('melsc-a',''.$urlp.'/js/mel/mscript-a.js',array('jquery'));
			wp_enqueue_style('melcss',''.$urlp.'/js/mel/mediaelementplayer.min.css', __FILE__);
			wp_enqueue_style('melcsss',''.$urlp.'/js/mel/mejs-skins.css', __FILE__);
			if ($ex_muz != "") {
			$musicaa .= "<div class='dIW'>";
			foreach ($dis as $di){
				$posti = get_post($di);
				$musicaa .= "<audio width='$jhw' height='30' style='width: 100%; height: 100%;'  controls src='".$posti->guid."' preload='none' />";
				$musicaa .= "<object width='$jhw' height='30' type='application/x-shockwave-flash' data='".$urlp."/js/mel/flashmediaelement.swf'>
							<param name='movie' value='".$urlp."/js/mel/flashmediaelement.swf' />
							<param name='flashvars' value='controls=true&file=".$posti->guid."' />
							</object>												
							</audio>";
			}
			$musicaa .="</div><div style='clear:both;'> </div>";
			}
		}
		return $musicaa;
	}
...
function getvideo_aa($atts) {
	...
	if ($opts['hidevideoall'] == 'no'){
		...
		if($vshow == 'yes' || $vshow == '') {
			...
			wp_enqueue_style('melcsss',''.$urlp.'/js/mel/mejs-skins.css', __FILE__);
			if ($ex_vid != "") {
			$videoaa .= "<div class='dIW'>";
			foreach ($dis as $di){
				$posti = get_post($di);
				$videoaa .= "<video width='$jhw' height='$jhh' style='width: 100%; height: 100%;' src='".$posti->guid."' controls preload='none'/>";
				$videoaa .= "<object width='$jhw' height='$jhh' type='application/x-shockwave-flash' data='".$urlp."/js/mel/flashmediaelement.swf'>
							<param name='movie' value='".$urlp."/js/mel/flashmediaelement.swf' />
							<param name='flashvars' value='controls=true&file=".$posti->guid."' />
							</object>
							</video>";
			}
			$videoaa .="</div><div style='clear:both;'> </div>";
			}
		}
		return $videoaa;
	}
